( function( $ ) {
	...
	$.fn.SlideDeckNew = function( options ) {

		// add zoom for wipe effect
		if( options.fx === "wipe" ){
			options.clip = "zoom";
		}
		options.slideResize = 0; //Stop slide resize.

		var slidedeck_frame = jQuery( this );
		var slides = jQuery( this ).find( 'li' );
		var parent_frame = jQuery( this ).parent( '.slidedeck-frame' );

		

		/**
		 *  pager builder callback for cycle
		 */

		options.pagerAnchorBuilder = function( idx, slide ) {
			var im = $( slide ).children( 'img.sd2-slide-background' );
			var html;
			if ( im.length > 0 ) {
				var thumbnail = '';
				if( $( slide ).data('thumbnail-src') != '' ) {
                                        thumbnail = $( slide ).data('thumbnail-src');
                                } else {
                                    if ( im[0].dataset.src === undefined ) {
                                            thumbnail = im[0].src;
                                    } else {
                                            thumbnail = im[0].dataset.src;
                                    }
                                }
				html = '<a class="prime-cycle-pager" href=#><span class="prime-nav-num-wrapper"><span class="prime-nav-num">' + ( idx + 1 ) + '</span></span><span class="prime-nav-tooltip"><span class="prime-nav-image" style="background-image: url(' + thumbnail + ');">&nbsp;</span></span></a>';
			} else {
				html = '<a class="prime-cycle-pager" href=#><span class="prime-nav-num-wrapper"><span class="prime-nav-num">' + ( idx + 1 ) + '</span></span></a>';
			}
			return html;
		}
		...
		var slidedeckAdjustImage = function( slide ) {
                        // Take outer width of slides parent container.
			var th = $( slide ).parent().height(),
					tw = $( slide ).parent().outerWidth(),
			...
			if ( th > 0 ) {
				if ( im.length > 0 ) {
					...
					// remove previous margintop fix for lightbox
					im.css( {marginTop: 0} );
					...
					if ( jQuery( slide ).hasClass( 'sd2-image-scaling-none' ) ) {
						...
						if( (ih*2 < th) && (jQuery( slide ).find('img').hasClass( 'sd2-is-first-slide' )) )
							{  
						...
						if ( jQuery( slide ).is( ':hidden' ) ) {
                                                        var slideOldWidth = jQuery( slide ).width();
                                                        jQuery( slide ).width('auto');
							jQuery( slide ).show();
							var nh = im.height(),
									nw = im.width(),
									hd = ( nh - th ) / 2,
									wd = ( nw - tw ) / 2;
							jQuery( slide ).hide();
                                                        jQuery( slide ).width(slideOldWidth);
						} else {
                                                    var nh = im.height(),
									nw = im.width(),
									hd = ( nh - th ) / 2,
		...
		var openOverlay = function( overlay ) {
			var overlays = jQuery( overlay ).parent().find( '.slidedeck-overlay' );
			var overlayWidth = 2;
			// calculate the width for overlay wrapper
			overlays.each( function( ind ) {
				overlayWidth += overlays.eq( ind ).outerWidth();
			} );
			jQuery( overlay ).parent().addClass( 'open' );
			jQuery( overlay ).parent().width( overlayWidth );
		}

		// close overlay function

		var closeOverlay = function( overlay ) {
			jQuery( overlay ).parent().removeClass( 'open' );
			jQuery( overlay ).parent().width( 0 );
		}

		/**
		 *
		 * Bind thickbox click event
		 */
		jQuery( parent_frame ).find( 'a.thickbox' ).bind( 'click', function( e ) {
			// reload slider
			slidedeck_frame.cycle( 'destroy' );

			var slidedeck_id = $( slidedeck_frame ).attr( 'id' );

			// remove previous effect changes
			$( slides ).css( {
				width: '',
				height: ''
			} );

			$( "#" + slidedeck_id ).cycle( options );

                        $( "#" + slidedeck_id ).find('li.active img.sd2-slide-background').addClass( 'slidedeck-adjust-width' ).removeClass( 'slidedeck-adjust-height' );

			$('#TB_window').addClass('open');
			/* Pager width calculator for prime lens*/
			$( '.lens-prime' ).each( function() {
				// get width of pager element
				var w = jQuery( this ).find( '#prime-custom-pager' ).width();
				// calculate the pager lenth
				var n = jQuery( this ).find( '#prime-custom-pager > a' ).length;

				jQuery( this ).find( '#prime-custom-pager > a' ).width( parseFloat( w/n ) - parseFloat( 5 )  );
			} );

		} );
		/**
		 * function to scan all thickbox url and replace with natural width
		 */
		$( 'a.thickbox' ).each( function() {
				var tburl=$(this).attr('href');
				var regEx = /([?&]z)=([^#&]*)/g;
				var thewidth=$(this).closest('.slidedeck-frame').outerWidth();
				var newurl=updateQueryStringParameter(tburl,'width',thewidth);
				$(this).attr('href',newurl);
		} );
		/**
		 * function to replace query string with key in url
		 * @param {type} uri
		 * @param {type} key
		 * @param {type} value
		 * @returns {String}
		...
		/**
		 *  Bind thickbox close event
		 */

		jQuery( 'body' ).on( 'click', '#TB_closeWindowButton', function( e ) {
			// reload slider
			slidedeck_frame.cycle( 'destroy' );
			...
			$( "#" + slidedeck_id ).cycle( options );
			$( '#TB_window' ).removeClass( 'open' );

		} );

                slidedeck_frame.has('.slide-type-video').each(function(){
                    if(typeof($.data(this, 'video-slidedeck')) == 'undefined'){
                        $.data(this, 'video-slidedeck', new SlideDeckVideoAPIs(this));
                    }
                });
                var primeLazyLoad = new SlideDeckLazyLoad(slidedeck_frame);

                // cycle prev next event
                options.onPrevNextEvent = function( isNext, zeroBasedSlideIndex, slideElement ) {
                    var slidedeck;
                    $( slidedeck_frame ).each( function() {
                        slidedeck = $.data( this, 'SlideDeckCover' );
                    } );
                    slidedeck = slidedeck.slidedeck;

                    if( slidedeck.former > 0 && slidedeck.former < 5  ) {slidedeck.former = slidedeck.former + 1;}else { slidedeck.former = 1; }

                    // For back cover.
                    slidedeck.current = zeroBasedSlideIndex + 1;
                    if( typeof(slidedeck.options.before) === 'function'){
                        slidedeck.options.before(slidedeck);
                    }
                    // For play/pause video on navigation.
                    if( typeof(slidedeck.options.complete) === 'function'){
                        slidedeck.options.complete(slidedeck);
                    }
			slidedeckAdjustImage( slideElement );
                    if(isNext) {
                        primeLazyLoad.lazyLoadNext(zeroBasedSlideIndex, parent_frame.data('sd2-lazy-load-padding') );
                    } else {
                        primeLazyLoad.lazyLoadPrev(zeroBasedSlideIndex, parent_frame.data('sd2-lazy-load-padding') );
                    }
                }
		// cycle after transition event
		options.after = function( currSlideElement, nextSlideElement, options, forwardFlag ) {
			slidedeckAdjustImage( currSlideElement );
		}
		options.before = function( currSlideElement, nextSlideElement, options, forwardFlag ) {
			slidedeckAdjustImage( nextSlideElement );

			// lazy load videos
			$( nextSlideElement ).find( 'div.sd2-slide-background' ).each( function() {
				var imgurl = $( this ).attr( 'data-sd2-slide-image' );
				$( this ).css( 'background-image', 'url(' + imgurl + ')' );

				if ( $( nextSlideElement ).hasClass( 'sd2-image-scaling-contain' ) ) {
					$( this ).css( 'background-size', 'contain' );
				} else if ( $( nextSlideElement ).hasClass( 'sd2-image-scaling-cover' ) ) {
		...
		$( "body" ).on( "keydown", function( e ) {
			if ( options.keyboard ) {
				// forword 39
				if ( e.keyCode === 39 ) {
					$( slidedeck_frame ).cycle( 'pause' );
					$( slidedeck_frame ).cycle( 'next' );
				} else if ( e.keyCode === 37 ) {
					// backword 37
					$( slidedeck_frame ).cycle( 'pause' );
					$( slidedeck_frame ).cycle( 'prev' );
				}
			}
		...
		slidedeck_frame.bind( "mousewheel", function( event, mousewheeldelta ) {
		...
		var parent_width = parent_frame.width();
		var parent_height = parent_frame.height();

		/*
		 *  Full Screen button click
		 */

		$( parent_frame ).on( 'click', '.fullscreen-wrapper', function() {

			
			var windowHeight = $( window ).height();
			var windowWidth = $( window ).width();
			var images = $( slidedeck_frame ).find( 'img.sd2-slide-background' );

			// hide lightbox in full screen

			$( parent_frame ).find( '.thickbox-link-wrapper' ).hide();

			// change src of each image
			$( images ).each(
					function() {
						var fullimage = $( this ).data( 'fullsrc' );
						var imgsrc = $( this ).attr( 'src' );
						// now swap src
						$( this ).attr( 'data-fullsrc', imgsrc );
			...
			parent_frame.css( parent_frame_css );

			var slidedeck_frame_css = {
				"width": "100%",
				"height": ( windowHeight - 40 ),
			};

			slidedeck_frame.css( slidedeck_frame_css );
			...
			$( this ).addClass( 'smallscreen-wrapper' );
			$( this ).removeClass( 'fullscreen-wrapper' );

			$('.lens-prime').addClass('open');
			...
			$( '.lens-prime' ).each( function() {
				...
				jQuery( this ).find( '#prime-custom-pager > a' ).width( parseFloat( w/n ) - parseFloat( 1 )  );
			} );

		} );

		/**
		 *   Small screen button click
		 */

		$( parent_frame ).on( 'click', '.smallscreen-wrapper', function() {

			var images = $( slidedeck_frame ).find( 'img.sd2-slide-background' );

			// show lightbox in full screen

			$( parent_frame ).find( '.thickbox-link-wrapper' ).show();
			...
			$( "#" + slidedeck_id ).cycle( options );
			$( this ).removeClass( 'smallscreen-wrapper' );
			$( this ).addClass( 'fullscreen-wrapper' );

			$( '.lens-prime' ).removeClass( 'open' );
		...
		jQuery( parent_frame ).ready( function() {
			if ( jQuery( parent_frame ).hasClass( 'sd2-overlays-open' ) ) {
				openOverlay( jQuery( parent_frame ).find( '.slidedeck-overlays-wrapper' ) );
			}
		} );
		// if autoplay is on set variable
		if ( options.timeout > 0 ) {
			SlideDeckCover.prototype.autoplay = true;
			SlideDeckCover.prototype.timeout = options.timeout;
		} else {
			SlideDeckCover.prototype.autoplay = false;
		}

		// add options to cover prototype
		SlideDeckCover.prototype.slideroptions = options;
		SlideDeckCover.prototype.slides = slides;

                var autoAdjustHeight = function(slide){
		...
		var adjustSlideHeight = function( slide ){
                    ...
                    if( parseInt(slide_actual_height) < 200 && screen.width > 700 ){
                            slide_actual_height = height;
                    } else if( parseInt(slide_actual_height) < 200 && screen.width < 700 ){
                            // set min height 200 for mobile devices
                            slide_actual_height = 200;
                    }
                    slide.parent().css('height', slide_actual_height + "px");
                ...
                jQuery( window ).load( function(e){
                    // adjust slidedeck height after load.
                    if( slides.length > 0 ){
                        for(var i=0; i<slides.length; i++){
                            autoAdjustHeight($(slides[i]));
                        }
                    }
                });
			...
			$( "#" + slidedeck_id ).cycle( options );
                    ...
                    //Hide back cover and reinit cycle.
                    $(".slidedeck-cover-back .slidedeck-cover-restart").bind('click', function(event) {
	...
	SlideDeckSkin['prime'] = function( slidedeck ) {
		var ns = 'prime';
		...
		//elems.frame.append('<div id="adv-custom-pager" class="center external"></div>');

		//jQuery("#prime-custom-pager").prepend('<div class="accent-bar-top accent-color-background">&nbsp;</div>');

		jQuery( elems.frame ).find( '.sd2-node-caption' ).append( '<div class="accent-bar-top accent-color-background">&nbsp;</div>' );

		jQuery( document ).on( 'hover', '.prime-nav-num',
				function() {
					$( this ).closest( '.prime-cycle-pager' ).find( '.prime-nav-tooltip' ).show();
				} );

		jQuery( document ).on( 'mouseout', '.prime-nav-num', function() {
			$( this ).closest( '.prime-cycle-pager' ).find( '.prime-nav-tooltip' ).hide();
		} );

		jQuery( '.prime-nav-num' ).hover( function() {
			$( this ).closest( '.prime-cycle-pager' ).find( '.prime-nav-tooltip' ).show();
		}, function() {
			$( this ).closest( '.prime-cycle-pager' ).find( '.prime-nav-tooltip' ).hide();
		} );
	...
	$( document ).ready( function() {
		...
		// hide lightbox icon for iframe
		var preview_iframe = parent.document.getElementById( 'slidedeck-preview' );
		if ( preview_iframe !== null ) {
			jQuery( '.fullscreen-wrapper' ).hide();
			jQuery( '.thickbox-link-wrapper' ).hide();
		}


		$( '.lens-prime .slidedeck' ).each( function() {
			if ( typeof ( $.data( this, 'lens-prime' ) ) == 'undefined' || $.data( this, 'lens-prime' ) == null ) {
				$.data( this, 'lens-prime', new SlideDeckSkin['prime']( this ) );
			}
		} );
	} );
