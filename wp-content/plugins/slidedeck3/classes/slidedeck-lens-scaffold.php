...
class SlideDeckLens_Scaffold {
    ...
    function _slidedeck_sizes( $sizes, $slidedeck ) {
        if( $this->is_valid( $slidedeck['lens'] ) && isset( $this->lens['meta']['sizes'] ) ){
            // remove fullwidth option for vanilla lens
            if( $this->lens['slug'] === 'vanilla' )
                unset($this->lens['meta']['sizes']['fullwidth']);
	...
	$dependencies = array( 'jquery', "{$this->namespace}-library-js" );

        if( isset( $this->lens['slug'] ) && in_array( $this->lens['slug'] , array( 'prime' , 'parfocal' ) ) ){
            array_push( $dependencies , "cycle-all" );
        }
