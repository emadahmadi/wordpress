...
//            ),
    ...
    function register_scripts() {
        ...
        if( file_exists( SLIDEDECK2_DIRNAME . $filename ) ) {
             wp_register_script( "slidedeck-deck-{$this->name}-admin", SLIDEDECK2_URLPATH . $filename, array( 'jquery', 'slidedeck-admin', $this->namespace.'-preview' ), SLIDEDECK2_VERSION, true );
        }
    }
...
final public function render( $id, $styles = array(), $include_lens_files = true, $preview = false, $echo_js = false, $start = false, $post = null, $front_page ) {
		...
		if( $slidedeck['options']['auto_height'] || $slug === "titles" || $slug === "polarad" || $slug === "prime" || $slug === "parfocal" ){
        ...
        if($SlideDeckPlugin->get_option( 'slidedeck_align_center' )) {
            $frame_styles_str .= ' margin: 0 auto; ';
        }
        $html = '';
        ...
        if( isset( $slidedeck['options']['size'] ) && "fullwidth" == $slidedeck['options']['size'] ) {
            $html = '<div class="slidedeck-fullwidth-wrapper" style="position: relative; width: 100%; height: auto;">';
            $show_fullwidth = true;
        }
		...
		if( ( $show_fullwidth ) || ( 'parfocal' != $slidedeck['lens'] && isset($slidedeck['options']['show_lightbox']) && $slidedeck['options']['show_lightbox'] ) ) {
            $html .= '</div>';
        }
        ...
        $vertical_properties = array(
            ...
            'scroll' => isset($slidedeck['options']['scroll']) ? $slidedeck['options']['scroll'] : false,
		...
		$javascript_options['scroll'] = isset($slidedeck['options']['scroll']) ? $slidedeck['options']['scroll'] : false;
		...
		if( $slidedeck['lens'] === 'prime' || $slidedeck['lens'] === 'parfocal' ){
			...
			$SlideDeckPlugin->footer_scripts .= '<script type="text/javascript">jQuery("#' . $slidedeck_unique_id . '").slidedeck( ' . json_encode( $javascript_options ) . ' )' . $vertical_scripts . ';</script>';
                ...
                if( $slidedeck['lens'] === 'layerpro' || $slidedeck['lens'] === 'parallax' ){
	...
	function render_dt_and_dd_elements( $slidedeck, $slides ) {
        ...
        foreach( $slides as $slide ) {
			...
			if( in_array($slidedeck['lens'], $new_layout_lenses) ){
				...
				if( $slide['type'] == "video" ){
					$output .= '<li itemscope="" itemtype="http://schema.org/VideoObject" style="' . $slide['styles'] . '" class="' . implode( " ", $slide['classes'] ) . '" data-thumbnail-src="' . $slide['thumbnail'] . '">';
				} else {
					$output .= '<li itemscope="" itemtype="http://schema.org/ImageObject" style="' . $slide['styles'] . '" class="' . implode( " ", $slide['classes'] ) . '" data-thumbnail-src="' . $slide['thumbnail'] . '">';
				}
			} else {
				$output .= "<dt". ( (!empty( $spine_classes ) ) ? ' class="'. implode( " ", $spine_classes ) . '"' :'') . ( (!empty( $spine_styles ) ) ? ' style="'. $spine_styles . '"' :'') .">{$slide_title}</dt>";
				if( $slide['type'] == "video" ){
					$output .= '<dd itemprop="video" itemscope itemtype="http://schema.org/VideoObject" style="' . $slide['styles'] . '" class="' . implode( " ", $slide['classes'] ) . '" data-thumbnail-src="' . $slide['thumbnail'] . '">';
				} else {
					$output .= '<dd itemscope="" itemtype="http://schema.org/ImageObject" style="' . $slide['styles'] . '" class="' . implode( " ", $slide['classes'] ) . '" data-thumbnail-src="' . $slide['thumbnail'] . '">';
				}
			}
    ...
    function render_overlays( $slidedeck, $slidedeck_unique_id, $post, $front_page ) {
        ...
        $front_page = isset($_REQUEST['front_page']) ? slidedeck2_sanitize( $_REQUEST['front_page'] ) : '';
        if( $front_page === "true" ) {
            $front_page = true;
        }
    ...
    final public function save( $id = null, $params = array() ) {
        ...
        } else if( 'parfocal' != $data['lens'] && in_array( $data['options']['size'], array( 'fullwidth', 'box') ) ) {
            $data['options']['size'] = 'medium';
        }
