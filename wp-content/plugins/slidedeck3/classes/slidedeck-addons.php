...
class SlideDeckAddons {
	...
	function get( $slug = "" )
	{
		...
		$addons_files = (array) glob( SLIDEDECK2_DIRNAME . '/slidedeck-addons/' . $folders . '/addons.json' );
		...
		if ( is_dir( SLIDEDECK3_ADDONS_DIR ) ) {
			...
			if ( is_readable( SLIDEDECK3_ADDONS_DIR ) ) {
				// Load and combine the custom addons in with the stock addons
				$custom_addons_files = (array) glob( SLIDEDECK3_ADDONS_DIR . '/' . $folders . '/addons.json' );
	...
	function is_protected( $addon_filename )
	{
		...
		if ( $file_exists ) {
			if ( str_replace( "\\", "/", dirname( dirname( $addon_filename ) ) ) == str_replace( "\\", "/", SLIDEDECK2_CUSTOM_LENS_DIR ) ) {
				$protected = false;
			}
		}
