...
class SlideDeck_Addons_Upload extends WP_Upgrader {
	...
	function install( $package ) {
		...
		if( !is_dir( SLIDEDECK3_ADDONS_DIR ) ) {
            if( is_writable( dirname( SLIDEDECK3_ADDONS_DIR ) ) ) {
                mkdir( SLIDEDECK3_ADDONS_DIR, 0755 );
            }
        }
		...
		$options = array(
			'package' => $package,
			'destination' => SLIDEDECK3_ADDONS_DIR,
	...
	function check_package( $source ) {
		...
		if( !is_writable( SLIDEDECK3_ADDONS_DIR ) )
...
class SlideDeck_Addon_Installer_Skin extends WP_Upgrader_Skin {
	...
	function after() {
		$install_actions = array(
			'return' => '<a href="' . slidedeck2_action( '/addons' ) . '" title="' . __( "Return to SlideDeck Addon Management", "slidedeck" ) . '">' . __( "Return to SlideDeck Addons Management", "slidedeck" ) . '</a>'
		);
