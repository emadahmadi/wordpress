...
/*
 ...
 Plugin URI: https//www.slidedeck.com/demo
...
class SlideDeckLitePlugin {
	...
	//include_once( SLIDEDECK2_DIRNAME . '/template/main.php' );
                ...
                foreach( $slidedecks as $slidedeck ) {
                    ...
                    if( in_array( "medialibrary", $sources ) && !in_array( "custom", $sources ) ) {
                        ...
                        foreach( $media_ids as $media_id ) {
                            ...
                            $slidedeck['options']['total_slides'] = 5;
    ...
    function admin_options_update( ) {
        ...
        $options = array(
            ...
            'slidedeck_align_center' => isset( $data['slidedeck_align_center'] ) && !empty( $data['slidedeck_align_center'] ) ? true : false
        );
    ...
    function ajax_change_lens( ) {
        ...
        $lenses = $this->get_slidedeck_lenses( $slidedeck );
        $is_prime = $this->slidedeck_is_prime();
 ...
 function ajax_template_modal( ) {
    	...
    	include (SLIDEDECK2_DIRNAME . '/views/elements/_template-modal.php');
    	exit ;
    }
    ...
    function discount_available_message() {
        if ( $this->is_plugin() && get_current_screen()->id != $this->menu['upgrades'] ) {
            ...
            if ( $days_left > 86400 ) {
                ...
                $message .= '<p><strong>Upgrade to SlideDeck3 in the next ' . $days_left . ' days and get 25% off -</strong> Like what you\'ve seen with SlideDeck3 Lite so far?. ';
            } else {
                $days_left = 0;
                $message .= '<p><strong>Last day to save 25% to upgrade to SlideDeck3! -</strong> Like what you\'ve seen with SlideDeck3 Lite so far?. ';
            }
            */
            $message .= '<p><strong>Like what you\'ve seen with SlideDeck3 Lite so far?. </strong>';
            $message .= '<a href="' . $this->action( '/upgrades' ) . '">Upgrade to SlideDeck3 Now</a>';
            $message .= '</p></div>';
    ...
    function slidedeck_is_prime() {
        // Get the list of slidedecks to check whether prime lens has been used in any of the slidedeck's
        $order_options = $this->order_options;
        $options = $this->order_options;
        $option_keys = array_keys( $options );
        $first_key = reset( $option_keys );
...
/**
    ...
    function route( ) {
        ...
        if( preg_match( "/admin\.php\?.*page\=" . SLIDEDECK2_BASENAME . "\/filament/", $uri ) ) {
            wp_redirect( "http://filament.io/applications/slidedeck-app?utm_source=sd3lite_wp&utm_medium=deployment&utm_content=admin&utm_campaign=filament" );
            exit ;
        }
    ...
    function slidedeck_manage_addons_after_addons( $addons ) {
        ...
        include( SLIDEDECK2_DIRNAME . '/views/upsells/_upsell-additional-addon-manage.php' );
    }
    ...
    function slidedeck_admin_rate_us($footer_text) {
        ...
        if($screen->parent_base === 'slidedeck2-lite') {
            
        $rate_text = sprintf(
			/* translators: %s: Link to 5 star rating */
			__( 'If you like <strong>SlideDeck 3 Lite</strong> please leave us a %s rating. It takes a minute and helps a lot. Thanks in advance!', 'give' ),
			'<a href="https://wordpress.org/support/view/plugin-reviews/slidedeck3?filter=5#postform" target="_blank" class="give-rating-link" data-rated="' . esc_attr__( 'Thanks :)', 'give' ) . '">&#9733;&#9733;&#9733;&#9733;&#9733;</a>'
		);
...
/**
 ...
 function slidedeck_create_new_slide_slidedeck_block( $html ) {
    	ob_start( );
    	include (SLIDEDECK2_DIRNAME . '/views/elements/_new_slider.php');
    	$html = ob_get_contents( );
    	ob_end_clean( );
 ...
 function slidedeck_create_slide_using_template_block( $html ) {
    	ob_start( );
    	include (SLIDEDECK2_DIRNAME . '/views/elements/_create-slide-using-template.php');
    	$html = ob_get_contents( );
    	ob_end_clean( );
    ...
    function wp_register_scripts( ) {
        // Admin JavaScript
        wp_register_script( "{$this->namespace}-admin", SLIDEDECK2_URLPATH . "/js/{$this->namespace}-admin" . (SLIDEDECK2_ENVIRONMENT == 'development' ? '.dev' : '.dev') . ".js", array( 'jquery', 'media-upload', 'slidedeck-fancy-form', $this->namespace . '-simplemodal' ), SLIDEDECK2_VERSION, true );
        // Lite Admin JavaScript
        wp_register_script( "{$this->namespace}-admin-lite", SLIDEDECK2_URLPATH . "/js/{$this->namespace}-admin-lite" . (SLIDEDECK2_ENVIRONMENT == 'development' ? '.dev' : '.dev') . ".js", array( 'slidedeck-admin' ), SLIDEDECK2_VERSION, true );
		...
		wp_register_script( 'jail', SLIDEDECK2_URLPATH . '/js/jail.js', array(), SLIDEDECK2_VERSION, true );
 		// cycle library
		wp_register_script( 'cycle-all', SLIDEDECK2_URLPATH . '/js/jquery.cycle.all.js', array(), SLIDEDECK2_VERSION, true );
    }
