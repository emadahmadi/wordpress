...
class SlideDeckSource_CustomContent extends SlideDeck {
	...
	function register_scripts()
	{
		...
		wp_register_script( "slidedeck-deck-{$this->name}-admin", SLIDEDECK2_URLPATH . '/sources/' . $this->name . '/source.js', array( 'jquery', 'slidedeck-admin', $this->namespace . '-preview' ), SLIDEDECK2_VERSION, true );
	}
	...
	function process_template( $nodes, $slidedeck, $deck_iteration )
	{
		...
		if ( isset( $slide_types[$slide_type] ) ) {
			...
			if ( file_exists( $template ) ) {
				...
				if ( isset( $video_meta ) && ( $slidedeck['lens'] === 'prime' || $slidedeck['lens'] === 'parfocal' || $slidedeck['lens'] === 'parallax' || $slidedeck['lens'] === 'tiled' || $slidedeck['lens'] === 'layerpro' ) ) {
